...
public class JwtService {
	...
	public boolean isValid(String token,UserDetails user)
	{
		...
		boolean validToken=tokenRepo.findByAccessToken(token).map(t -> !t.isExpired() && !t.isRevoked()).orElse(false);
	 ...
	 public boolean isValidRefreshToken(String token,UserDetails user)
	 {
		...
		boolean validRefreshToken=tokenRepo.findByRefreshToken(token).map(t -> !t.isExpired() && !t.isRevoked()).orElse(false);
   ...
   public String generateRefreshToken(ApiUsers user)
	{
